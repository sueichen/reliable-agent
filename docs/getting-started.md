...
技能通过斜杠命令（如 `/ra-plan`）或 AI 的 Skill 工具调用。你只需用自然语言告诉 AI 要做什么，AI 会自动调用正确的技能。
...
对 AI 说："使用 ra-verify 运行验证"

运行完整验证：测试、lint、构建、类型检查。全部通过才能进入审查。

### 5. 审查

对 AI 说："使用 ra-request-review 进行代码审查"

并行运行 5 个专业审查者（正确性、安全、测试覆盖、性能、代码风格），综合生成审查报告。

### 6. 处理审查反馈

对 AI 说："使用 ra-receive-review 处理审查反馈"

系统性修复每条发现，Critical 必须有证明测试，修复后重验证。

### 7. 发布

对 AI 说："使用 ra-ship 准备发布"

提交校验 + PR 生成 + CI 监控 + 合并。等待你批准后推送。

### 8. 回顾与进化

对 AI 说："使用 ra-evolve 进行 session 回顾"
...
- 对 AI 说"使用 ra-spec 初始化项目"开始任何新项目
- 对 AI 说"使用 ra-plan 设计这个功能"规划任何新功能
- AI 会在每个步骤引导你
