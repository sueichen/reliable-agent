...
│  内容：错误模式、优化发现、审查高频问题         │
│  进化方式：reliable-agent:ra-evolve Phase 1 自动追加             │
│  风险：低（只记录，不改变行为）               │
├─────────────────────────────────────────────┤
...
│  进化方式：reliable-agent:ra-evolve 读取经验 → 生成建议 │
│  风险：中（改变行为，但范围受限）               │
├─────────────────────────────────────────────┤
│  第三层：Skills（命令技能）                    │
│  角色：AI 可提议，人类审批后生效               │
...
│  内容：工作流步骤、门禁规则、验证标准          │
│  进化方式：reliable-agent:ra-evolve 生成变更建议       │
│  风险：高（改变核心行为，必须人工确认）         │
└─────────────────────────────────────────────┘
```
...
```
reliable-agent:ra-evolve (每个 session 结束，或周期性)
    │
