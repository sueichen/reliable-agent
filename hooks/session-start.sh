...
if ! using_skill_content=$(cat "${PLUGIN_ROOT}/skills/using-reliable-agent/SKILL.md" 2>/dev/null); then
  echo "[WARN] SessionStart: cannot read using-reliable-agent skill: ${PLUGIN_ROOT}/skills/using-reliable-agent/SKILL.md" >&2
  using_skill_content="Error reading using-reliable-agent skill"
fi

using_skill_escaped=$(escape_for_json "$using_skill_content")

# ---- Build session context ----
session_context="<HARD-GATE>
...
When a project contains a .reliable-agent/experiences.md file, read it when debugging, reviewing, or before making changes in areas with recorded experiences — it contains structured knowledge of past error patterns, optimization discoveries, and recurring review issues. When working within a specific skill, also read .reliable-agent/<skill-name>/experiences.md for skill-specific experiences.
</IMPORTANT>
</HARD-GATE>"
