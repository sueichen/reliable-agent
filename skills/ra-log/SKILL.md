...
<HARD-GATE>
1. 绝不在没有遥测信号的情况下声称功能完成——每个新功能/端点/外部调用必须有对应的结构化日志 + RED 指标
2. 绝不使用非结构化日志（console.log/print/echo）——所有日志输出必须是结构化 JSON，含稳定事件名和关联 ID
3. 绝不在日志行中输出密钥、令牌、PII 或完整请求体——日志在写入前必须通过安全检查
4. 绝不跳过遥测验证——必须通过诱导错误或检查 staging 输出来验证每条遥测信号端到端可达
</HARD-GATE>
