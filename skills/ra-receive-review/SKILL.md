---
name: ra-receive-review
description: "处理代码审查反馈——系统性修复 Critical 和 Important 发现，记录 Optional 项，修复后重新验证。在收到 ra-request-review 或外部审查者的审查报告后使用。"
version: "1.0.0"
license: MIT
---
...
digraph reliable_receive_review {
    ...
    start [label="启动 /ra-receive-review", shape=doublecircle];
    parse [label="解析审查报告\n提取所有发现"];
...
### Step 6: 门禁检查
- 如果所有 Critical/Important 已解决→标记就绪，准备 ra-ship
...
**所有 Critical 已修复时**:
- `/ra-evolve` — 从本次审查反馈中提取可复用的经验教训

**仍有 Critical 待处理时**:
- `/ra-build` → `/ra-verify` — 修复剩余问题并重新验证
- 修复完成后重新运行 `/ra-request-review` 获取新一轮审查
