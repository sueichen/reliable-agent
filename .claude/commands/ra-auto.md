...
---
Invoke the reliable-agent:ra-auto skill.
...
5. 修复后自动重新验证（ra-verify）和重新审查（ra-request-review）
6. 审查通过后自动执行 update-doc（自动更新文档）
7. 在 update-doc 完成后停止，生成自动决策报告
8. 提示用户手动运行 /ra-ship 和 /ra-evolve
...
注意: 此命令不从 ra-spec 开始（规范生成始终需要人工交互）。
注意: 此命令不执行 ra-evolve（进化建议需在积累经验后单独运行）。
注意: 此命令不执行 ra-ship（发布始终需要人类批准）。
